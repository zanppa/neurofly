...
class Fly:
    ...
    def calc(self):
        ...
        # Try with linear rotation
        self.ang += 2.0*react[0] - 1.0
        self.plaa.append(2.0*react[0] - 1.0)
        
        # And also speed can be determined from the response
        fwd = react[1] * speed

        xnew = self.cx[-1] + fwd * np.sin(np.deg2rad(self.ang))
        ynew = self.cy[-1] + fwd * np.cos(np.deg2rad(self.ang))
        self.cx.append(xnew)
        self.cy.append(ynew)
